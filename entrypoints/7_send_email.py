...
from group4py.src.constants.prompts import DEFAULT_QUESTIONS
...
# Function to sanitize tag names and values
    ...
    def find_latest_pdf_factsheet(self) -> Optional[str]:
        ...
        try:
            ...
            if not outputs_dir.exists():
                print(f"⚠️ Primary outputs directory not found: {outputs_dir}")
                ...
                if not outputs_dir.exists():
                    print(f"❌ No valid directory with PDFs found after checking alternatives")
                    return None
            ...
            if not pdf_files:
                print(f"No PDFs found directly in {outputs_dir}, searching subdirectories...")
                pdf_files = list(outputs_dir.glob("**/*.pdf"))
            ...
            print(f"📄 Found latest PDF factsheet: {latest_pdf.name} (Modified: {latest_pdf.stat().st_mtime})")
            ...
            print(f"✅ PDF validation passed: {latest_pdf.name} ({latest_pdf.stat().st_size} bytes)")
    ...
    def send_email_with_attachments(
        ...
        try:
            ...
            if not pdf_attachments:
                print("⚠️ Warning: No PDF attachments found, checking outputs/factsheets folder...")
            ...
            for idx, attachment in enumerate(pdf_attachments):
                print(f"Processing PDF attachment {idx+1}/{len(pdf_attachments)}: {attachment.get('filename')}")
                ...
                try:
                    ...
                    if isinstance(attachment['content'], bytes):
                        ...
                        print(f"  - Binary content: {len(attachment['content'])} bytes → base64 encoded")
                    else:
                        # Convert string content to bytes then base64
                        content_b64 = base64.b64encode(attachment['content'].encode('utf-8')).decode('utf-8')
                        print(f"  - Text content: {len(attachment['content'])} chars → base64 encoded")
                    ...
                    print(f"  ✗ Error processing PDF attachment {attachment.get('filename')}: {str(e)}")
                    import traceback
                    traceback.print_exc()
            ...
            print(f"  - to: {type(email_payload['to']).__name__} with {len(recipient_value) if isinstance(recipient_value, list) else 1} recipient(s)")
            ...
            print(f"📧 Sending email with {len(processed_attachments)} PDF attachment(s) to: {', '.join(recipients)}")
            print(f"  - Attachments: {', '.join([a.get('filename', 'unnamed') for a in processed_attachments])}")
...
def main():
    try:
        ...
        all_recipients = set([r.strip() for r in default_recipients.split(',') if r.strip()])
        if additional_recipients:
            all_recipients.update([r.strip() for r in additional_recipients.split(',') if r.strip()])
