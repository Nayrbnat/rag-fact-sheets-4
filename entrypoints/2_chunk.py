#!/usr/bin/env python3
"""
Script for extracting text from PDFs and creating document chunks.
"""
...
def get_file_paths():
    ...
    logger.info(
        f"[3_CHUNK] Found {len(pdf_files)} PDF files in {pdf_directory}"
    )
    ...
    try:
        ...
        with db.get_session() as session:
            ...
            if is_processed and not force_reprocess:
                logger.info(
                    f"[3_CHUNK] Document {doc_id} has already been processed. Skipping."
                )
                return None
            ...
            if force_reprocess and document:
                ...
                try:
                    ...
                    chunk_ids_query = select(DocChunkORM.id).filter(
                    ...
                    if deleted_relationships > 0:
                        logger.info(
                            f"[3_CHUNK] Deleted {deleted_relationships} relationships "
                            f"for document {doc_id}"
                        )
                    
                    # Now delete the chunks
                    deleted_chunks = session.query(DocChunkORM).filter(
                        DocChunkORM.doc_id == doc_uuid
                    ).delete()
                    logger.info(
                        f"[3_CHUNK] Removed {deleted_chunks} existing chunks "
                        f"for document {doc_id}"
                    )
                    ...
                    logger.error(
                        f"[3_CHUNK] Error removing existing chunks and relationships: {e}"
                    )
            ...
            if not document:
                logger.info(
                    f"[3_CHUNK] Document {doc_id} not found in database. Creating record..."
                )
                ...
                try:
                    ...
                    if len(parts) > 2 and len(parts[2]) >= 8:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"[3_CHUNK] Could not parse date from filename: {parts[2]}"
                            )
            ...
            for strategy in extraction_strategies:
                try:
                    logger.info(
                        f"[3_CHUNK] Attempting text extraction with strategy: {strategy}"
                    )
                    raw_extracted_elements = extract_text_from_pdf(
                    ...
                    if raw_extracted_elements and len(raw_extracted_elements) > 0:
                        logger.info(
                            f"[3_CHUNK] Strategy {strategy} extracted "
                            f"{len(raw_extracted_elements)} elements"
                        )
                        
                        # Handle different element structures - some may be dicts, others objects
                        text_content = []
                        for elem in raw_extracted_elements:
                            try:
                                ...
                                logger.warning(
                                    f"[3_CHUNK] Error processing element with strategy "
                                    f"{strategy}: {elem_error}"
                                )
                                continue
                        
                        if text_content:
                            logger.info(
                                f"[3_CHUNK] Successfully extracted {len(text_content)} "
                                f"text elements using {strategy} strategy"
                            )
                            extracted_elements = text_content
                            break  # Success! Stop trying other strategies
                        else:
                            logger.warning(
                                f"[3_CHUNK] Strategy {strategy} returned elements "
                                f"but no actual text content"
                            )
                    else:
                        logger.warning(
                            f"[3_CHUNK] Strategy {strategy} returned no elements"
                        )
            ...
            if not extracted_elements:
                logger.error(
                    f"[3_CHUNK] All text extraction strategies failed for {file_path}"
                )
                
                # Create a minimal fallback chunk with filename information
                fallback_text = (
                    f"Document: {Path(file_path).name}\n"
                    f"Note: Text extraction failed - document may be "
                    f"image-based or corrupted."
                )
            ...
            logger.info(
                f"[3_CHUNK] Final extracted elements count: {len(extracted_elements)}"
            )
            ...
            for i, chunk in enumerate(cleaned_chunks):
                ...
                if not chunk.get('text', '').strip():
                    logger.warning(
                        f"[3_CHUNK] Skipping empty chunk {i} from {file_path}"
                    )
                    continue
            ...
            logger.info(
                f"[3_CHUNK] Attempting to upload {len(db_chunks)} chunks to database "
                f"for {file_path}"
            )
            upload_success = upload(session, db_chunks, table='doc_chunks')
            if not upload_success:
                logger.error(
                    f"[3_CHUNK] Failed to upload chunks to database for {file_path}"
                )
                return None
                
            logger.info(
                f"[3_CHUNK] Uploaded {len(db_chunks)} chunks to doc_chunks table"
            )

            # 6. Update documents table that the document has been processed (chunked)
            update_processed(
                session, NDCDocumentORM, doc_id, 
                chunks=cleaned_chunks, table='documents'
            )
            logger.info(
                f"[3_CHUNK] Updated processed status in documents table for {doc_id}"
            )
...
async def run_script(force_reprocess: bool = False):
    ...
    try:
        ...
        if not file_paths:
            logger.warning("[3_CHUNK] No PDF files found to process")
            return
        ...
        valid_chunks = [
            chunks for chunks in chunk_results 
            if chunks is not None and len(chunks) > 0
        ]
        
        if not valid_chunks:
            logger.warning("[3_CHUNK] No valid chunks created")
        else:
            total_chunks = sum(len(chunks) for chunks in valid_chunks)
            logger.warning(
                f"[3_CHUNK] All files processed successfully. {total_chunks} chunks created"
            )
            logger.warning(
                "[3_CHUNK] Chunking script completed successfully. Ready for embedding."
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Extract text and create chunks from PDF files"
    )
    parser.add_argument(
